(function initPatientForm() {
    ...
    // ================= STRICT DATE FORMATTING LOGIC =================
    // Forces DD-MM-YYYY consistently everywhere 
    ...
    // Prevents timezone shifting when parsing raw strings like '2026-02-13'
    ...
    const mainAppContent = getEl("mainAppContent");
    ...
    const welcomeDoctorName = getEl("welcomeDoctorName");
    ...
    async function authFetch(url, options = {}) {
        ...
        const updatedOptions = { ...options, headers: { ...headers, ...options.headers } };
    ...
    function checkAuth() {
        if (doctorToken) {
            ...
            welcomeDoctorName.textContent = `| Dr. ${doctorName}`;
            startApp(); 
        } else {
    ...
    async function startApp() {
        ...
        const headerDate = getEl("headerDate");
        ...
        // Set Header Date - Removed so it doesn't show next to the logout button
        // if (headerDate) {
        //     headerDate.textContent = getFormattedDate(new Date());
        // }

        // Master Date Setter
        function setVisitDate(dateObj) {
            if (!visitDate) {
                console.error("Date input not found. Make sure your HTML input has id='visitDate'");
                return; 
            }
            
            // Safely format to YYYY-MM-DD for local timezone
            const year = dateObj.getFullYear();
            const month = String(dateObj.getMonth() + 1).padStart(2, '0');
            const day = String(dateObj.getDate()).padStart(2, '0');
            const yyyy_mm_dd = `${year}-${month}-${day}`;
            
            visitDate.value = yyyy_mm_dd;
            visitDate.setAttribute("data-date", getFormattedDate(dateObj));
        }

        if (visitDate) {
            visitDate.addEventListener("change", function() {
                this.setAttribute("data-date", getFormattedDate(parseLocalDate(this.value)));
            });
        }
        ...
        if(closeTestsModalBtn) closeTestsModalBtn.addEventListener("click", () => testsModal.style.display = "none");
        if(testCloseBtn) testCloseBtn.addEventListener("click", () => testsModal.style.display = "none");
        ...
        // ================= AUTO-SUGGESTIONS (DEBOUNCED & EVENT DELEGATED) =================
        const handleMobileInput = debounce(async (event) => {
            let el = event.target;
            ...
            try {
                const res = await authFetch(`${API_BASE_URL}/api/visits/mobile-suggestions?query=${encodeURIComponent(query)}`);
                ...
                if (results.length > 0) {
                    results.forEach(item => {
                        ...
                        li.dataset.mobile = item.B_Mobile; // Store data for delegation
            ...
            } catch (err) { console.error("Mobile suggestions error:", err); }
        }, 300);

        if (mobileInput) mobileInput.addEventListener("input", handleMobileInput);

        // Mobile Event Delegation
        if (mobileSuggestionsList) {
            mobileSuggestionsList.addEventListener("click", async (e) => {
                if (e.target.tagName === "LI") {
                    const selectedMobile = e.target.dataset.mobile;
                    mobileInput.value = selectedMobile; 
                    mobileSuggestionsList.classList.add("hidden"); 
                    checkOldRecordButton();
                    try {
                        const searchRes = await authFetch(`${API_BASE_URL}/api/visits/search?mobile=${encodeURIComponent(selectedMobile)}`);
                        const searchData = await searchRes.json();
                        if(searchData.records && searchData.records.length > 0) {
                            autoFillPatientDetails(searchData.records[0]);
                        }
                    } catch(err) { console.error("Mobile fill error:", err); }
                }
            });
        }

        const handleNameInput = debounce(async (event) => {
            let el = event.target;
            checkOldRecordButton();
            const query = el.value.trim();
            if (query.length < 1) { suggestionsList.classList.add("hidden"); return; }
            try {
                const res = await authFetch(`${API_BASE_URL}/api/visits/suggestions?query=${encodeURIComponent(query)}`);
                ...
                if (names.length > 0) {
                    names.forEach(item => {
                        ...
                        li.dataset.name = item.B_PName; // Store data for delegation
                        suggestionsList.appendChild(li);
                    });
                    suggestionsList.classList.remove("hidden");
                } else { suggestionsList.classList.add("hidden"); }
            } catch (err) { console.error("Name suggestions error:", err); }
        }, 300);

        if (patientNameInput && suggestionsList) patientNameInput.addEventListener("input", handleNameInput);
        ...
        if (suggestionsList) {
            suggestionsList.addEventListener("click", async (e) => {
                if (e.target.tagName === "LI") {
                    ...
                    checkOldRecordButton();
                    try {
                        const searchRes = await authFetch(`${API_BASE_URL}/api/visits/search?name=${encodeURIComponent(selectedName)}`);
                        const searchData = await searchRes.json();
                        if(searchData.records && searchData.records.length > 0) {
                            autoFillPatientDetails(searchData.records[0]);
                        }
                    } catch(err) { console.error("Name fill error:", err); }
                }
            });
        }
        ...
        document.addEventListener("click", function(e) { 
            if (e.target !== mobileInput && mobileSuggestionsList) mobileSuggestionsList.classList.add("hidden"); 
            if (e.target !== patientNameInput && suggestionsList) suggestionsList.classList.add("hidden"); 
        });
        ...
        function fillForm(record) {
            ...
            if (visitDate && record.B_Date) {
                setVisitDate(new Date(record.B_Date));
            }
        ...
        if (saveBtn) {
            saveBtn.addEventListener("click", (e) => {
                ...
                showModal('confirm', 'Confirm Save', 'Are you sure you want to save this record?', async () => {
                    ...
                    try {
                        const res = await authFetch(`${API_BASE_URL}/api/visits`, { method: "POST", body: JSON.stringify(getPayload()) });
                        ...
                        showModal('alert', 'Error', 'Server Error. Check console for details.'); 
                    } 
        ...
        if (updateBtn) {
            updateBtn.addEventListener("click", (e) => {
                ...
                showModal('confirm', 'Confirm Update', 'Are you sure you want to update this record?', async () => {
                    ...
                    try {
                        const res = await authFetch(`${API_BASE_URL}/api/visits/${snoInput.value}`, { method: "PUT", body: JSON.stringify(getPayload()) });
        ...
        if (deleteBtn) {
            deleteBtn.addEventListener("click", (e) => {
                ...
                showModal('delete', 'Confirm Delete', `Delete record #${snoInput.value}?`, async () => {
                    ...
                    try {
                        const res = await authFetch(`${API_BASE_URL}/api/visits/${snoInput.value}`, { method: "DELETE" });
        ...
        if (saveAsNewBtn) {
            saveAsNewBtn.addEventListener("click", (e) => {
                ...
                showModal('confirm', 'Save as New Record', 'Create a NEW visit with today\'s date using these details?', async () => {
                    ...
                    try {
                        const res = await authFetch(`${API_BASE_URL}/api/visits`, { method: "POST", body: JSON.stringify(payload) });
        ...
        if (printBtn) {
            printBtn.addEventListener("click", () => {
                ...
                const printDate = getFormattedDate(parseLocalDate(visitDate.value || new Date().toISOString().split('T')[0]));
                ...
                printWindow.document.write(`<div class="info-grid"><div>NAME: ${name.toUpperCase()}</div><div>DATE: ${printDate}</div></div>`);
        ...
        async function loadVisitsForPatient(name, mobile) {
            ...
            try {
                const res = await authFetch(`${API_BASE_URL}/api/visits/search?${queryParam}`);
                const data = await res.json();
                ...
                data.records.forEach(rec => {
                    const date = getFormattedDate(new Date(rec.B_Date)); 
                    const row = document.createElement("tr");
                ...
                showModal('alert', 'Error', 'Could not load visits.'); 
            }
        }
        ...
        if (showAllBtn) {
            showAllBtn.addEventListener("click", async () => {
                try {
                    const res = await authFetch(`${API_BASE_URL}/api/visits/all-patients`);
                    const records = await res.json();
                    renderPatientList(records);
                } catch (err) { 
                    console.error("Show All Error:", err);
                    showModal('alert', 'Error', 'Error fetching patients list.'); 
                }
            });
        }
        ...
        async function loadNextSno() {
            try { 
                const res = await authFetch(`${API_BASE_URL}/api/visits/next-sno`); 
                const data = await res.json(); 
