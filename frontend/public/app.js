...
(function initPatientForm() {
    ...
    // Wrapper function to auto-attach the JWT Token to all requests
    async function authFetch(url, options = {}) {
        const headers = {
            "Content-Type": "application/json",
            "Authorization": `Bearer ${doctorToken}` // THE KEY
        };
        const updatedOptions = { ...options, headers: { ...headers, ...options.headers } };
        
        const response = await fetch(url, updatedOptions);
        
        // If token expired or invalid, log them out automatically
    ...
    function checkAuth() {
        if (doctorToken) {
            ...
            startApp(); // Start the app ONLY if logged in
        } else {
    ...
    async function startApp() {
        ...
        /* NOTE: ALL fetch() CALLS BELOW ARE NOW USING authFetch() */
                    ...
                    if (results.length > 0) {
                        results.forEach(item => {
                            ...
                            li.onclick = async () => {
                                ...
                                checkOldRecordButton();
                                try {
                                    const searchRes = await authFetch(`${API_BASE_URL}/api/visits/search?mobile=${encodeURIComponent(item.B_Mobile)}`);
                                    const searchData = await searchRes.json();
                                    if(searchData.records && searchData.records.length > 0) {
                                        autoFillPatientDetails(searchData.records[0]);
                                    }
                    ...
                    if (names.length > 0) {
                        names.forEach(item => {
                            ...
                            li.onclick = async () => {
                                ...
                                checkOldRecordButton();
                                try {
                                    const searchRes = await authFetch(`${API_BASE_URL}/api/visits/search?name=${encodeURIComponent(item.B_PName)}`);
                                    const searchData = await searchRes.json();
                                    if(searchData.records && searchData.records.length > 0) {
                                        autoFillPatientDetails(searchData.records[0]);
                                    }
        ...
        async function loadNextSno() {
            try { const res = await authFetch(`${API_BASE_URL}/api/visits/next-sno`); const data = await res.json(); if (res.ok) snoInput.value = data.nextSno; } catch (err) { console.error(err); }
        }
